...
//            config.status = WifiConfiguration.Status.ENABLED;
            ...
            config.allowedAuthAlgorithms.set(AuthAlgorithm.OPEN);//该配置支持的身份验证协议集合     公认的IEEE 802.11认证算法
            config.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.TKIP);//该配置所支持的组密码集合       公认的组密码
            config.allowedKeyManagement.set(KeyMgmt.WPA_PSK);//该配置所支持的密钥管理集合      公认的密钥管理方案
