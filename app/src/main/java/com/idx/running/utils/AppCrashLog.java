...
public abstract class AppCrashLog implements Thread.UncaughtExceptionHandler {
        ...
        try {
            ...
            if(file != null && file.isDirectory()) {
                ...
                if(files != null && files.length > 10) {
                    ...
                    for(int i = 0 ; i < files.length - LIMIT_FILE_COUNT;i++) files[i].delete();
                }
            }
        } catch (Exception e) {
    ...
    private class FileLogFilter implements FileFilter {
        ...
        public boolean accept(File pathname) {
            if(pathname !=null && pathname.getName().endsWith(".log")) return true;
            return false;
        }
    }
    ...
    private void collectDeviceInfo(Context context) {
        try {
            ...
            /**
             * 2,获取手机型号，系统版本，以及SDK版本
             */
            crashLogInfos.put("手机型号:", android.os.Build.MODEL);
            crashLogInfos.put("系统版本", ""+android.os.Build.VERSION.SDK);
            crashLogInfos.put("Android版本", android.os.Build.VERSION.RELEASE);
